...
// ═══════════════════════════════════════════════════
// NUCLEO: update() e render() — unici punti di scrittura e disegno
// ═══════════════════════════════════════════════════
//
// REGOLA D'ORO:
//   - DATA è l'UNICA fonte di verità.
//   - Ogni modifica ai dati passa da update().
//   - Ogni aggiornamento dello schermo passa da render().
//   - render() è SEMPRE sicuro da chiamare, anche mentre l'utente
//     digita: preserva focus e cursore, non distrugge mai i campi attivi.
//   - Il listener Firebase in arrivo chiama solo: applica dati → render().
//     Non servono più flag come _isSaving, _firstLoad, recentlyTyped.

let _saveTimer = null;

// Unico punto di SCRITTURA. fn modifica DATA, poi si salva e si ridisegna.
function update(fn) {
...
// Salvataggio con debounce. Snapshot del codice campo per evitare
// che un salvataggio in ritardo finisca sul campo sbagliato.
function scheduleSave() {
  try { localStorage.setItem('cambusa_local_' + (CAMPO_CODE||'draft'), JSON.stringify(DATA)); } catch(e){}
  if (!CAMPO_CODE || !FB_READY) { setSyncStatus('local'); return; }
  setSyncStatus('saving');
  clearTimeout(_saveTimer);
  const codeAtSave = CAMPO_CODE;
  _saveTimer = setTimeout(() => {
    if (CAMPO_CODE !== codeAtSave) return; // campo cambiato nel frattempo
    fbDb.ref('campi/' + codeAtSave).set(DATA)
      .then(()  => setSyncStatus('saved'))
      .catch(() => setSyncStatus('error'));
  }, 600);
}

// Unico punto di DISEGNO. Legge DATA e aggiorna il DOM in modo idempotente.
// Sicuro da chiamare sempre — le funzioni di render preservano il focus.
function render() {
  if (!SECTION) return;
  loadConfIntoForm();
  buildMenu();
...
// ── Listener live ─────────────────────────────────
let _fbListenerRef = null;  // riferimento Firebase catturato alla connessione

function stopListener() {
  if (fbListener && _fbListenerRef) {
    _fbListenerRef.off('value', fbListener);
    fbListener = null;
    _fbListenerRef = null;
  }
}
...
function connectCampo() {
  ...
  fbListener = ref.on('value', snapshot => {
    if (CAMPO_CODE !== codeAtConnect) return; // campo cambiato
...
// ── Save ──────────────────────────────────────────
// saveData() è ora un alias di scheduleSave() (compatibilità con il codice esistente)
...
// REFRESH COMPLETO
// ═══════════════════════════════════════════════════
// refreshAll() è ora un alias di render() (compatibilità con il codice esistente)
